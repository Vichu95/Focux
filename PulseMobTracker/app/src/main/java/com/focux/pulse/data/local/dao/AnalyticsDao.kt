...
interface AnalyticsDao {
    ...
    // Effective opens = raw SESSION_APP entries - FOCUS_RETAINED events, floored at 0
    @Query("""
        SELECT MAX(0,
            COUNT(CASE WHEN type = 'SESSION_APP' THEN 1 END) - COUNT(CASE WHEN type = 'FOCUS_RETAINED' THEN 1 END)
        )
        FROM app_sessions 
